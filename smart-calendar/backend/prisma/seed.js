 ...
 * 사용법:
 *   npx prisma db seed          → DB에 시드 삽입
 *   node prisma/seed.js --export → DB 없이 JSON만 생성
 */
const fs = require('fs');
...
function exportSeedJson() {
  ...
  const seedJsonPath = path.join(outputDir, 'seed-events.json');
  fs.writeFileSync(seedJsonPath, JSON.stringify(sampleEvents.map(e => ({
    ...e,
...
// --export 모드: DB 없이 JSON만 생성
if (process.argv.includes('--export')) {
  const seedJsonPath = exportSeedJson();
  console.log(`📦 시드 데이터 JSON export 완료: ${seedJsonPath}`);
  process.exit(0);
}
...
async function main() {
  ...
  // JSON으로도 export
  const seedJsonPath = exportSeedJson();
  console.log(`📦 시드 데이터 JSON export: ${seedJsonPath}`);
}
